 ...
 */
class MathUtil {
	public static inline var STANDARD_PI:Float = 3.14159265359;
	...
	public static inline function lerp(a:Float, b:Float, ratio:Float, ?adjustToFramerate:Bool = false, ?framerate:Float = 60):Float {
		if (adjustToFramerate)
			ratio = MathUtil.adjustToFramerate(ratio, framerate);
		return a + ratio * (b - a);
	}
	 ...
	 */
	public static function pointInRectangle(pointX:Float, pointY:Float, rect:Rect2D):Bool {
		return pointX >= rect.x && pointX <= rect.w && pointY >= rect.y && pointY <= rect.h;
	}
	 ...
	 */
	public static inline function multMatrices(vec:Array<Float>, mat:Array<Array<Float>>) {
	 ...
	 * @author @colleen05
	 */
	public static inline function letterBoxRectangle(contentSize:Vector2D, container:Rect2D) {
		// Calculate aspect ratios.
		var innerAspect:Float = contentSize.x / contentSize.y;
		var containerAspect:Float = container.w / container.h;
		...
		var boxWidth:Float = (containerAspect > innerAspect) ? // Container proportionally wider than inner?
			(container.h * innerAspect) : // Only take up a portion of the width.
			(container.w); // Otherwise, take up the entire width.

		var boxHeight:Float = (containerAspect < innerAspect) ? // Container proportionally taller than inner?
			(container.w / innerAspect) : // Only take up a portion of the height.
			(container.h); // Otherwise, take up the entire height.

		// Create the new rectangle, positioned properly inside the container.
		var box = Rl.Rectangle.create(container.x + (container.w - boxWidth) / 2.0, // Centered horizontally.
			container.y + (container.h - boxHeight) / 2.0, // Centered vertically.
