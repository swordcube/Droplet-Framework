...
import droplet.backend.macros.MacroUtil;

/**
 * Class representing a color, based on Int. Provides a variety of methods for creating and converting colors.
 *
 * DropletColors can be written as Ints. This means you can pass a hex value such as
 * 0xff123456 to a function expecting a DropletColor, and it will automatically become a DropletColor "object".
 * Similarly, DropletColors may be treated as Ints.
 *
 * Note that when using properties of a DropletColor other than ARGB, the values are ultimately stored as
...
abstract DropletColor(Int) from Int to Int {
	 ...
	 * You can add more colors for `DropletColor.fromString(String)` if you need.
	 */
	public static var colorLookup(default, null):Map<String, Int> = MacroUtil.buildMap("droplet.backend.graphics.DropletColor");
	 ...
	 */
	public var rgb(get, set):DropletColor;
	 ...
	 * @return	The color as a DropletColor
	 */
	public static function fromInt(Value:Int):DropletColor {
		return new DropletColor(Value);
	}
	 ...
	 * @return The color as a DropletColor
	 */
	public static function fromRGB(Red:Int, Green:Int, Blue:Int, Alpha:Int = 255):DropletColor {
		var color = new DropletColor();
	 ...
	 * @return The color as a DropletColor
	 */
	public static function fromRGBFloat(Red:Float, Green:Float, Blue:Float, Alpha:Float = 1):DropletColor {
		var color = new DropletColor();
	 ...
	 * @return The color as a DropletColor
	 */
	public static function fromCMYK(Cyan:Float, Magenta:Float, Yellow:Float, Black:Float, Alpha:Float = 1):DropletColor {
		var color = new DropletColor();
	 ...
	 * @return	The color as a DropletColor
	 */
	public static function fromHSB(Hue:Float, Saturation:Float, Brightness:Float, Alpha:Float = 1):DropletColor {
		var color = new DropletColor();
	 ...
	 * @return	The color as a DropletColor
	 */
	public static function fromHSL(Hue:Float, Saturation:Float, Lightness:Float, Alpha:Float = 1):DropletColor {
		var color = new DropletColor();
		return color.setHSL(Hue, Saturation, Lightness, Alpha);
	}

	/**
	 * Parses a `String` and returns a `DropletColor` or `null` if the `String` couldn't be parsed.
	 *
	 ...
	 * @return	A `DropletColor` or `null` if the `String` couldn't be parsed
	 */
	public static function fromString(str:String):Null<DropletColor> {
		var result:Null<DropletColor> = null;
		str = StringTools.trim(str);

		if (COLOR_REGEX.match(str)) {
			var hexColor:String = "0x" + COLOR_REGEX.matched(2);
			result = new DropletColor(Std.parseInt(hexColor));
			...
			for (key in colorLookup.keys()) {
				if (key.toUpperCase() == str) {
					result = new DropletColor(colorLookup.get(key));
					break;
				}
			}
		}
	 ...
	 * @return	HSB color wheel as Array of DropletColors
	 */
	public static function getHSBColorWheel(Alpha:Int = 255):Array<DropletColor> {
	 ...
	 */
	public static function interpolate(Color1:DropletColor, Color2:DropletColor, Factor:Float = 0.5):DropletColor {
	 ...
	 */
	public static function gradient(Color1:DropletColor, Color2:DropletColor, Steps:Int, ?Ease:Float->Float):Array<DropletColor> {
		var output = new Array<DropletColor>();
	...
	/**
	 * Multiply the RGB channels of two DropletColors
	 */
	@:op(A * B)
	public static function multiply(lhs:DropletColor, rhs:DropletColor):DropletColor {
		return DropletColor.fromRGBFloat(lhs.redFloat * rhs.redFloat, lhs.greenFloat * rhs.greenFloat, lhs.blueFloat * rhs.blueFloat);
	}

	/**
	 * Add the RGB channels of two DropletColors
	 */
	@:op(A + B)
	public static function add(lhs:DropletColor, rhs:DropletColor):DropletColor {
		return DropletColor.fromRGB(lhs.red + rhs.red, lhs.green + rhs.green, lhs.blue + rhs.blue);
	}

	/**
	 * Subtract the RGB channels of one DropletColor from another
	 */
	@:op(A - B)
	public static function subtract(lhs:DropletColor, rhs:DropletColor):DropletColor {
		return DropletColor.fromRGB(lhs.red - rhs.red, lhs.green - rhs.green, lhs.blue - rhs.blue);
	}
	 ...
	 */
	public function getComplementHarmony():DropletColor {
	...
	public function getSplitComplementHarmony(Threshold:Int = 30):Harmony {
		...
		var warmer:DropletColor = fromHSB(MathUtil.wrap(oppositeHue - Threshold, 0, 350), saturation, brightness, alphaFloat);
		var colder:DropletColor = fromHSB(MathUtil.wrap(oppositeHue + Threshold, 0, 350), saturation, brightness, alphaFloat);
	...
	public function getTriadicHarmony():TriadicHarmony {
		var triadic1:DropletColor = fromHSB(MathUtil.wrap(Std.int(hue) + 120, 0, 359), saturation, brightness, alphaFloat);
		var triadic2:DropletColor = fromHSB(MathUtil.wrap(Std.int(triadic1.hue) + 120, 0, 359), saturation, brightness, alphaFloat);
	 ...
	 */
	public function to24Bit():DropletColor {
	 ...
	 */
	public function getDarkened(Factor:Float = 0.2):DropletColor {
		Factor = MathUtil.bound(Factor, 0, 1);
		var output:DropletColor = this;
	 ...
	 */
	public function getLightened(Factor:Float = 0.2):DropletColor {
		Factor = MathUtil.bound(Factor, 0, 1);
		var output:DropletColor = this;
	 ...
	 * @return This color
	 */
	public inline function setRGB(Red:Int, Green:Int, Blue:Int, Alpha:Int = 255):DropletColor {
		red = Red;
		green = Green;
		blue = Blue;
		alpha = Alpha;
	 ...
	 * @return This color
	 */
	public inline function setRGBFloat(Red:Float, Green:Float, Blue:Float, Alpha:Float = 1):DropletColor {
		redFloat = Red;
		greenFloat = Green;
		blueFloat = Blue;
		alphaFloat = Alpha;
	 ...
	 * @return This color
	 */
	public inline function setCMYK(Cyan:Float, Magenta:Float, Yellow:Float, Black:Float, Alpha:Float = 1):DropletColor {
	 ...
	 * @return	This color
	 */
	public inline function setHSB(Hue:Float, Saturation:Float, Brightness:Float, Alpha:Float):DropletColor {
	 ...
	 * @return	This color
	 */
	public inline function setHSL(Hue:Float, Saturation:Float, Lightness:Float, Alpha:Float):DropletColor {
	 ...
	 */
	inline function setHueChromaMatch(Hue:Float, Chroma:Float, Match:Float, Alpha:Float):DropletColor {
		Hue %= 360;
		var hueD = Hue / 60;
	...
	inline function set_rgb(value:DropletColor):DropletColor {
		validate();
		this = (this & 0xff000000) | (value & 0x00ffffff);
		return value;
	}

	function get_rgb():DropletColor {
